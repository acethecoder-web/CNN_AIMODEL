...
class XRaySorterModel(nn.Module):
    ...
    def forward(self, x): 
        ...
        x = x.view(-1, 128 * 28 * 28)  # Flatten tensor for fully connected layer
        x = F.relu(self.fc1(x))
        x = self.dropout(x)
        x = F.relu(self.fc2(x))
        x = self.dropout(x)
...
def evaluate_model(model, test_loader):
    ...
    with torch.no_grad():
        for images, labels in test_loader:
            images, labels = images.to(device), labels.to(device)  # Move tensors to correct device
            outputs = model(images)
...
if __name__ == '__main__':
    ...
    transform = transforms.Compose([
        transforms.Resize((224, 224)),  # Fixed: Resize images to 224x224
